...
while game_is_on:
    time.sleep(ball.time_sleep)  #==> make the ball moves slower: pause the loop for a short time during each iteration. ==> get our while loop to sleep for a little bit in between each of the updates. ==> Also, I can think of it as getting the ball to move on every refresh of the screen.
    ...
    if ball.distance(r_paddle) < 50 and 330 < ball.xcor() < 350:
        # print("Make contact")
        ball.bounce_x()
        scoreboard.r_score_plus_1()
        # ball.increase_speed()
        # ball.x_move *= 1.1
        # ball.y_move *= 1.1
        # Or, alternatively, I can modify "time":
        ball.time_sleep -= 0.001

    if ball.distance(l_paddle) < 50 and -350 < ball.xcor() < -330:
        ball.bounce_x()
        scoreboard.l_score_plus_1()
        # ball.increase_speed()
        # ball.x_move *= 1.1
        # ball.y_move *= 1.1
        # Or, alternatively, I can modify "time":
